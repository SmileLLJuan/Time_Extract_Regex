...
class TimeUnit:
    ...
    def norm_setSpecial(self):
        ...
        rule = u"[0-9]?[0-9]?[0-9]{2}-((10)|(11)|(12)|([0]?[1-9]))-((?<!\\d))([0-3][0-9]|[1-9])"
        pattern = re.compile(rule)
        match = pattern.search(self.exp_time)
        if match is not None:
            tmp_target = match.group()
        ...
        #英文日期  month/day/year  (年必须是4位)
        rule = u"((10)|(11)|(12)|([0]?[1-9]))(/|\\.)((?<!\\d))([0-3][0-9]|[1-9])(/|\\.)[0-2][0-9][0-9]{2}"
        pattern = re.compile(rule)
        match = pattern.search(self.exp_time)
        if match is not None:
            tmp_target = match.group()
    ...
    def norm_setCurRelated(self):
        ...
        rule = u"(?<=((?<!上)上个?(周|星期|礼拜)?(周|星期|礼拜)))[1-7]{1}"
        pattern = re.compile(rule)
        match = pattern.search(self.exp_time)
        if match is not None:
            flag[2] = True
            ...
            if cur.datetime.day<current_date.datetime.day:
                cur = arrow.get(arrow.get(arrow.now()).format('YYYY-M-D-H-m-s'), "YYYY-M-D-H-m-s")
        ...
        rule = "(?<=((?<!(本|这))(本|这)个?(周|星期)?(周|星期)))[1-7]{1}"
        pattern = re.compile(rule)
        match = pattern.search(self.exp_time)
        if match is not None:
            flag[2] = True
